...
# 把vue文件夹拖进VSCode
npm i         # 下载祖传依赖（耗时≈等校巴）
npm run dev   # 启动前端（比行政楼办事快点儿）
```

⚠️ **重要**：打开`vue/src/api/config.ts`修改`baseURL`值（本地运行可忽略）

### 后端启动（Spring Boot版）

1. 用IDEA打开`springboot`文件夹
2. 修改`src/main/resources/application.yaml`数据库密码：
3. 点击运行（祈祷别报错）
